...
import { Recurso } from 'src/app/domain/recurso.domain';
...
export class RecursoService {
  ...
  guardar(recurso: Recurso) {
    ...
    if (recurso !== undefined && recurso.id !== undefined && Number(recurso.id) !== 0 ) {
     return this.actualizar(recurso);
    } else {
      return this.http
        .post(url, recurso)
        .pipe(
  ...
  actualizar(recurso: Recurso) {
    const url = `${this.serviceBaseURL}/${this.pathEntityResource}/${recurso.id}` ;
  ...
  findAll(page: number , size: number , termino: string, multiSortMeta: SortMeta[]) {
    ...
    return this.http.get(url).pipe(
      map((recursos: any) => {
        this.commonService.normalizePropertyDate(recursos);
        return recursos;
      }),
      catchError((error: HttpErrorResponse) => this.handleError(error))); ;
  }

  findById(id: number): Observable<Recurso> {
    const url = `${this.serviceBaseURL}/${this.pathEntityResource}/${id}` ;
    const params = this.createHttpParams({});
    return this.http.get<Recurso>(url, { params })
              .pipe(
                map((recurso: any) => {
                  if (recurso.fechaBaja !== undefined &&
                    recurso.fechaBaja !== null) {
                    recurso.fechaBaja = DateUtils.convertStringToDate(recurso.fechaBaja);
                  }
                  return recurso;
                }),
                catchError((error: HttpErrorResponse) => this.handleError(error)));
  ...
  borrar(recurso: Recurso) {
    recurso.fechaBaja = new Date();
    return this.actualizar(recurso);
   }
}
